...
// 从日志条目推断严重级别（用于 Linear 风级别色点）
export type LogLevel = 'info' | 'warn' | 'error' | 'debug'

export function detectLogLevel(entry: LogEntry): LogLevel {
  const text = String(entry?.line || '')
  const source = String(entry?.source || '')
  if (/\b(ERROR|ERR|CRIT|FATAL|FAIL|EXCEPTION|panic)\b/i.test(text)) return 'error'
  if (/\b(WARN|WARNING|WARNING:)\b/i.test(text)) return 'warn'
  if (/\b(DEBUG|DBG|TRACE|VERBOSE)\b/i.test(text)) return 'debug'
  if (/\b(INFO|LOG|NOTICE)\b/i.test(text)) return 'info'
  // 无显式关键字时，按 source 推断
