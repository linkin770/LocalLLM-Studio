...
interface ChatInputProps {
  ...
  /** 重启服务；可传 overrideConfig 让重启使用最新配置（解决 setState 异步导致的旧值问题） */
  onRestartServer?: (overrideConfig?: Record<string, unknown>) => Promise<void>
...
export function ChatInput({
  ...
  const handleModelSelect = async (model: typeof modelsList[0]) => {
    // 注意：setState 是异步的，如果服务在跑，必须把"新 config"显式传给 restartServer，
    //       否则 restartServer 会从闭包里读到旧 state.config，新模型不会生效
    ...
    // 如果服务正在运行，重启以加载新模型 —— 用我们刚构造好的新 config
    if (isServerRunning && onRestartServer) {
      try {
        await onRestartServer(newConfig)
      ...
      <Sender
        ...
        footer={(actionNode) => (
          <div className="composer-footer">
            ...
            <div className="composer-right">{actionNode}</div>
          </div>
        )}
            suffix={false}
/>
