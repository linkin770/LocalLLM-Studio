...
export interface LlamaDesktopAPI {
  ...
  // 流式对话请求
  streamChat(opts: { requestId: string; sessionId?: string; config: Record<string, unknown> | null; messages: Array<{ role: string; content: string }> }): Promise<{ content: string; raw?: { usage?: { completion_tokens: number; total_tokens: number } } }>
  // 中止当前对话
...
export interface AppState {
  ...
  status: Status // 服务状态
  logs: LogEntry[] // 日志列表
  view: 'chat' | 'knowledge' | 'settings' // 当前视图（对话、知识库或设置）
