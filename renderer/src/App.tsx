...
function App() {
  ...
  // 使用 useRef 保存最新的流式请求 ID、消息列表和当前会话 ID，避免闭包陷阱
  ...
  const sendChat = useCallback(async (inputContent: string) => {
    ...
    // 如果没有内容和附件，或者正在忙碌，则直接返回（用 ref 做即时锁，防止同帧双击绕过）
    if ((!content && state.attachments.length === 0) || chatBusyRef.current)
      return;
    ...
    // 如果启用了知识库，检索相关内容并注入（查询过短时不搜索，避免低质量匹配）
    let knowledgeContext = '';
    const knowledgeEnabled = Boolean(state.config?.knowledgeEnabled)
    if (knowledgeEnabled && content.trim().length >= 2 && state.knowledgeDocuments.some(d => d.status === 'ready')) {
      try {
        const searchResult = await window.llamaDesktop.searchKnowledge(content, { topK: 3 })
        if (searchResult.results && searchResult.results.length > 0) {
          knowledgeContext = '\n\n【知识库参考】\n' +
            searchResult.results.map((r: any, idx: number) =>
    ...
    try {
      ...
      // 如果出错，移除空的助手消息，用 toast 提示错误（不加入 chatMessages，避免污染上下文）
      const current = chatMessagesRef.current;
      if (current.length && current[current.length - 1].role === 'assistant' && current[current.length - 1].streaming) {
        setChatMessages(current.slice(0, -1));
      }
      setToast(friendlyErrorMessage(error as string | Error));
      saveCurrentSession();
    }
    finally {
  ...
  const abortChat = useCallback(async () => {
    ...
    setChatBusy(false);
    setStreamRequestId('');
  }, [setChatBusy, setStreamRequestId]);
  ...
  const retryMessage = useCallback(async (index: number) => {
    ...
    try {
      ...
      // 处理错误：移除空助手消息，用 toast 提示（不加入 chatMessages，避免污染上下文）
      const current = chatMessagesRef.current;
      if (current.length && current[current.length - 1].role === 'assistant' && current[current.length - 1].streaming) {
        setChatMessages(current.slice(0, -1));
      }
      setToast(friendlyErrorMessage(error as string | Error).replace(/^发送失败/, '重试失败'));
      saveCurrentSession();
    }
    finally {
      chatBusyRef.current = false;
  ...
  }, [state.chatBusy, state.chatMessages, state.config, currentSessionPrompt, selectedSkill, setChatMessages, addChatMessage, setStreamRequestId, setChatBusy, updateChatMessage, saveCurrentSession, setToast]);
  ...
  // 编辑消息（仅用户消息）— 截断该 user 及其后所有消息，内容回输入框
  const editMessage = useCallback(async (index: number) => {
    ...
    // 截断到该 user（不含），该 user 内容回输入框
    setChatMessages(state.chatMessages.slice(0, index));
    saveCurrentSession();
    updateChatInput(message.content || '');
    clearAttachments();
    if (message.attachments?.length) addAttachments(message.attachments);
  }, [state.chatMessages, state.chatBusy, abortChat, setChatMessages, saveCurrentSession, updateChatInput, clearAttachments, addAttachments]);

  // 删除消息 — 级联删除配对消息，删除最后一对时用户消息回输入框
  const deleteMessage = useCallback(async (index: number) => {
    const target = state.chatMessages[index];
    if (!target) return;
    // 如果正在流式生成，先中断
    if (state.chatBusy || target.streaming) {
      await abortChat();
    }
    // 级联：删 assistant 时同时删其前的 user，删 user 时同时删其后的 assistant
    const removeIndices = new Set([index]);
    if (target.role === 'assistant' && index > 0 && state.chatMessages[index - 1].role === 'user') {
      removeIndices.add(index - 1);
    } else if (target.role === 'user' && index + 1 < state.chatMessages.length && state.chatMessages[index + 1].role === 'assistant') {
      removeIndices.add(index + 1);
    }
    const remaining = state.chatMessages.filter((_, i) => !removeIndices.has(i));
    // 如果删除的是最后一对 user+assistant，用户消息内容回输入框
    if (target.role === 'assistant' && removeIndices.has(index - 1)) {
      const userMsg = state.chatMessages[index - 1];
      updateChatInput(userMsg.content || '');
      clearAttachments();
      if (userMsg.attachments?.length) addAttachments(userMsg.attachments);
    } else if (target.role === 'user') {
      updateChatInput(target.content || '');
      clearAttachments();
      if (target.attachments?.length) addAttachments(target.attachments);
    }
    setChatMessages(remaining);
    saveCurrentSession();
  }, [state.chatMessages, state.chatBusy, setChatMessages, saveCurrentSession, abortChat, updateChatInput, clearAttachments, addAttachments]);
  ...
  useEffect(() => {
    ...
    // 同步 chatBusyRef：当外部（如 useAppState）设置 chatBusy=false 时，ref 也需同步
    if (!state.chatBusy) chatBusyRef.current = false;
  });
  ...
  useEffect(() => {
    ...
    const handleEvent = (payload: any) => {
      if (payload.type === 'status') {
        ...
        // 只对正在流式生成的 assistant 消息追加内容，防止删除后 delta 写入错误位置
        if (!target || target.role !== 'assistant' || !target.streaming) return;
        // 处理流式增量内容 - 即时更新
        if (payload.delta) {
  ...
  return (
    ...
    <div className={`app-shell ${state.sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <Sidebar sessions={state.sessions} currentSessionId={state.currentSessionId} historySearch={state.historySearch} historyMenuId={state.historyMenuId} sidebarCollapsed={state.sidebarCollapsed} view={state.view} chatMessages={state.chatMessages} status={state.status} busy={state.busy} onNewChat={startFreshSession} onFocusChat={() => setView('chat')} onShowKnowledgeBase={() => setView('knowledge')} onSearchChange={setHistorySearch} onOpenSession={openSession} onToggleHistoryMenu={(id) => setHistoryMenuId(state.historyMenuId === id ? '' : id)} onEditSession={editSession} onExportSession={exportSession} onDeleteSession={removeSession} onToggleSettings={() => { setView('settings'); setSettingsOpen(true); if (state.active !== 'overview') setActive('overview'); }} onToggleSidebar={() => setSidebarCollapsed(!state.sidebarCollapsed)} onSave={save} onStart={start} onStop={stop}/>
      ...
      <main className="main-area">
        ...
        {state.view === 'settings' ? (
          <SettingsPanel settingsOpen={state.settingsOpen} active={state.active} config={state.config} validation={state.validation} status={state.status} logs={state.logs} dirty={state.dirty} launch={state.launch} onClose={() => { setSettingsOpen(false); setView('chat'); }} onSelectSection={setActive} onUpdateConfig={updateConfig} onPickFile={pickFile} onCopyLaunchCommand={() => { const preview = (state.launch as Record<string, string>).preview; if (preview) { navigator.clipboard.writeText(preview); setToast('命令已复制'); } }} onOpenModelInfo={openModelInfo} sidebarCollapsed={state.sidebarCollapsed} onToggleSidebar={() => setSidebarCollapsed(!state.sidebarCollapsed)}/>
